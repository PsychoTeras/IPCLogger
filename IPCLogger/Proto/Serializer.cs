...
namespace IPCLogger.Proto
{
    internal static unsafe class Serializer
    {
        ...
        public static void Read(byte* bData, out void* data, ref int pos)
        {
            ...
            {
                data = Win32.HeapAlloc(size);
                Win32.Copy(data, &bData[pos], size);
                pos += size;
            }
        }
        ...
        public static void Write(byte* bData, void* val, int size, ref int pos)
        {
            *(int*)&bData[pos] = size;
            pos += sizeof(int);
            if (size != 0)
            {
                Win32.Copy(&bData[pos], val, size);
                pos += size;
            }
        }
